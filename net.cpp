...
// dorabmon: DNS seeds for reliable network bootstrap
...
// dorabmon: Hardcoded seed nodes (fallback if DNS fails)
unsigned int pnSeed[] =
{
...
bool BindListenPort(string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf("Unable to bind to port %d on this computer.  dorabmon is probably already running.", ntohs(sockaddr.sin_port));
        else
