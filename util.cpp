...
{
    string strConfigFile = GetDataDir() + "/dorabmon.conf";
...
void GetDataDir(char* pszDir)
{
    ...
    else
    {
        ...
        if (pszCachedDir[0] == 0)
        {
            ...
            if (pszHome == NULL || pszHome[0] == '\0')
                pszHome = "C:/";
            snprintf(pszCachedDir, sizeof(pszCachedDir), "%s/dorabmon", pszHome);
#else
            // Unix/Linux: Use HOME
            const char* pszHome = getenv("HOME");
            if (pszHome == NULL || pszHome[0] == '\0')
                pszHome = "/";
            snprintf(pszCachedDir, sizeof(pszCachedDir), "%s/.dorabmond", pszHome);
#endif
#endif
#if defined(_WIN32) || defined(__MINGW32__) || defined(__WXMSW__)
