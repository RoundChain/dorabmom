...
{
    return MyGetSpecialFolderPath(CSIDL_STARTUP, true) + "\\dorabmon.lnk";
}
...
void SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart)
    {
        ...
        printf("To start dorabmon on login, manually create a shortcut in your Startup folder.\n");
    }
}
#else
...
bool CMyApp::Initialize(int& argc, wxChar** argv)
{
    ...
    if (fDaemon)
    {
        ...
        // (actually, let's keep the current directory for .dorabmon access)
...
#endif
// dorabmon: Updated app name for data directory
#if defined(__WXMSW__ ) || defined(__WXMAC__)
    SetAppName("dorabmon");
#else
    SetAppName("dorabmond");
#endif
#if !defined(_WIN32) && !defined(__MINGW32__) && !defined(__WXMSW__)
    umask(077);
#endif
    ...
    // Load locale/<lang>/LC_MESSAGES/dorabmon.mo language file
    ...
    if (mapArgs.count("-?") || mapArgs.count("--help"))
    {
        wxString strUsage = string() +
          _("Usage:") + "\t\t\t\t\t\t\t\t\t\t\n" +
            "  dorabmon [options]       \t" + "\n" +
            "  dorabmon [command]       \t" + _("Send command to dorabmond running with -server or -daemon\n") +
            "  dorabmon [command] --help\t" + _("Get help for a command\n") +
            "  dorabmon help            \t" + _("List commands\n") +
          _("Options:\n") +
        ...
        if (fWindows && fGUI)
        {
            ...
            wxMessageBox(strUsage, "dorabmon", wxOK);
        }
        else
        {
            // Remove tabs
    ...
    if (GetBoolArg("-recover"))
    {
        ...
        {
            wxMessageBox(_("Database recovery failed. You may need to delete database files manually."), "dorabmon", wxOK | wxICON_ERROR);
            return false;
        }
        ...
        {
            wxMessageBox(_("Wallet key recovery failed. Check console for details."), "dorabmon", wxOK | wxICON_ERROR);
            return false;
        }
    ...
    wxString strMutexName = wxString("dorabmon_running.") + getenv("HOMEPATH");
    ...
    if (psingleinstancechecker->IsAnotherRunning())
    {
        ...
        loop
        {
            ...
            HWND hwndPrev = FindWindowA("wxWindowClassNR", "dorabmon");
            if (hwndPrev)
            {
    ...
    {
        wxMessageBox(strErrors, "dorabmon");
        return false;
    }
    ...
    //
    if (fDaemon)
        fprintf(stdout, "dorabmon server starting\n");
    strErrors = "";
    int64 nStart;
    ...
    if (mapArgs.count("-proxy"))
    {
        ...
        {
            wxMessageBox(_("Invalid -proxy address"), "dorabmon");
            return false;
        }
    }
    ...
    if (!CreateThread(StartNode, NULL))
        wxMessageBox("Error: CreateThread(StartNode) failed", "dorabmon");
...
{
    wxMessageBox(_("Program has crashed and will terminate.  "), "dorabmon", wxOK | wxICON_ERROR);
}
...
bool AppInit(int argc, char* argv[])
{
    ...
    if (mapArgs.count("-?") || mapArgs.count("--help"))
    {
        string strUsage = string() +
          _("Usage:") + "\n" +
            "  dorabmond [options]         \n" +
            "  dorabmond [command]         " + _("Send command to dorabmond running with -server or -daemon\n") +
            "  dorabmond [command] --help  " + _("Get help for a command\n") +
            "  dorabmond help              " + _("List commands\n") +
          _("Options:\n") +
            "  -gen              " + _("Generate coins\n") +
            "  -gen=0            " + _("Don't generate coins\n") +
            "  -genproclimit=<n> " + _("Limit mining to n processors (-1 = all)\n") +
