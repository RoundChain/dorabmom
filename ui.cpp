...
void CMainFrame::OnButtonNew(wxCommandEvent& event)
{
    ...
    choices.Add(_("Create New dorabmon Address"));
    choices.Add(_("Create New ok-Address"));
    choices.Add(_("Import Private Key (dorabmon Address)"));
...
CAddressBookDialog::CAddressBookDialog(wxWindow* parent, const wxString& strInitSelected, int nPageIn, bool fDuringSendIn) : CAddressBookDialogBase(parent)
{
    ...
    m_listCtrlReceiving->InsertColumn(1, _("dorabmon Address"), wxLIST_FORMAT_LEFT, 350);
#ifndef __WXMSW__
...
void CMyTaskBarIcon::Show(bool fShow)
{
    ...
    if (fShow)
    {
        ...
        if (nNumConnections == 0)
            strTooltip = strprintf(_STR("dorabmon - Offline (%d blocks)").c_str(), nBestHeight + 1);
        else if (nHighestPeerHeight > 0 && nBestHeight < nHighestPeerHeight - 5)
            strTooltip = strprintf(_STR("dorabmon - Syncing %d/%d").c_str(), nBestHeight + 1, nHighestPeerHeight);
        else
            strTooltip = strprintf(_STR("dorabmon - %d blocks").c_str(), nBestHeight + 1);
...
wxMenu* CMyTaskBarIcon::CreatePopupMenu()
{
    ...
    pmenu->Append(ID_TASKBAR_RESTORE, _("&Open dorabmon"));
