...
typedef HANDLE dorabmon_thread_t;
// Define cross-platform thread handle type
typedef dorabmon_thread_t thread_handle_t;

inline dorabmon_thread_t CreateThread(void(*pfn)(void*), void* parg, bool fWantHandle=false)
{
    DWORD nUnused = 0;
    HANDLE hthread =
        CreateThread(
    ...
    if (hthread == NULL)
    {
        ...
        return (dorabmon_thread_t)0;
    }
    if (!fWantHandle)
    {
        CloseHandle(hthread);
        return (dorabmon_thread_t)-1;
    }
    return hthread;
}
