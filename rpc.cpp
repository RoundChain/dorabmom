...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "stop\n"
            "Stop dorabmon server.");

    // Shutdown will take long enough that the response should get back
    CreateThread(Shutdown, NULL);
    return "dorabmon server stopping";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <dorabmonaddress>\n"
            "Return information about <dorabmonaddress>.");
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "getnewaddress [label]\n"
            "Returns a new dorabmon address for receiving payments.  "
...
Value setlabel(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setlabel <dorabmonaddress> <label>\n"
...
Value getaddressesbylabel(const Array& params, bool fHelp)
{
    ...
    CRITICAL_BLOCK(cs_mapAddressBook)
    {
        foreach(const PAIRTYPE(string, string)& item, mapAddressBook)
        {
            ...
            if (strName == strLabel)
            {
                // We're only adding valid dorabmon addresses and not ip addresses
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <dorabmonaddress> [minconf=1]\n"
            "Returns the total amount received by <dorabmonaddress> in transactions with at least [minconf] confirmations.");

    // dorabmon address
    string strAddress = params[0].get_str();
    CScript scriptPubKey;
    if (!scriptPubKey.SetBitcoinAddress(strAddress))
        throw runtime_error("Invalid dorabmon address");
...
Value getreceivedbylabel(const Array& params, bool fHelp)
{
    ...
    CRITICAL_BLOCK(cs_mapAddressBook)
    {
        foreach(const PAIRTYPE(string, string)& item, mapAddressBook)
        {
            ...
            if (strName == strLabel)
            {
                // We're only counting our own valid dorabmon addresses and not ip addresses
                CScript scriptPubKey;
                if (scriptPubKey.SetBitcoinAddress(strAddress))
                    if (IsMine(scriptPubKey))
...
Value ListReceived(const Array& params, bool fByLabels)
{
    ...
    CRITICAL_BLOCK(cs_mapWallet)
    {
        for (map<uint256, CWalletTx>::iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            foreach(const CTxOut& txout, wtx.vout)
            {
                // Only counting our own dorabmon addresses and not ip addresses
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "Each key can be a hex public key or a dorabmon address (if the key is in the wallet).\n"
...
Value createrawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "Output keys are dorabmon addresses, \"data\" for OP_RETURN hex, or raw hex scriptPubKey.");
...
void ThreadRPCServer2(void* parg)
{
    ...
    int nRPCPort = GetIntArg("-rpcport", 19759);
    ...
    try
    {
        ...
        if (result.type() != null_type)
        {
            if (fWindows && fGUI)
                ...
                MyMessageBox(strResult.c_str(), "dorabmon", wxOK);
            else
                fprintf(stdout, "%s\n", strResult.c_str());
        }
        return 0;
